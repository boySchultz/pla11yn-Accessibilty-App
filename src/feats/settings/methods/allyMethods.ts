...
export const setLineHeight = (params: AllyMethodParameters) => {
	const { ref, step, getSettingsState } = params;
	...
	if (settingsState?.initialValue === undefined) {
		...
		ref.current?.injectJavaScript(`document.body.style.lineHeight = '${lineSpacing(step ?? settingsState.activeStep)}em';`);
	}
};

export const setParagraphHeight = (params: AllyMethodParameters) => {
	const { ref, step, getSettingsState } = params;
	...
	if (settingsState?.initialValue === undefined) {
		...
		ref.current?.injectJavaScript(`
			...
			// 1.4.8(AAA): paragraph spacing is at least 1.5 times larger than the line spacing.
			if (${settingsState.activeStep} !==0  && ${step} !== 0 && lineHeight15 > ${paragraphHeight(settingsState.activeStep)}) {
				paragraphs.forEach((p) => {
					p.style.marginBottom = lineHeight15 + 'em';
				});
			}	else {
...
export const setLetterSpacing = (params: AllyMethodParameters) => {
	const { ref, step, getSettingsState } = params;
	...
	if (settingsState?.initialValue === undefined) {
		...
		ref.current?.injectJavaScript(`document.body.style.letterSpacing = '${letterSpacing(step ?? settingsState.activeStep)}';`);
	}
};

export const setWordSpacing = (params: AllyMethodParameters) => {
	const { ref, step, getSettingsState } = params;
...
export const setFontSize = (params: AllyMethodParameters) => {
	const { ref, step, getSettingsState } = params;
	...
	if (settingsState?.initialValue === undefined) {
		...
		ref.current?.injectJavaScript(`document.body.style.fontSize = '${fontSize(step ?? settingsState.activeStep)}' + 'px';`);
	}
};

export const setTextAlignment = (params: AllyMethodParameters) => {
		const { ref, step, getSettingsState } = params;
