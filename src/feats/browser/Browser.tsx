...
import {Appbar, Button, Drawer, TextInput, useTheme} from 'react-native-paper';
...
const Browser = () => {
	...
	const isValidURL = (url: string) => {
		const pattern = /^(https?:\/\/)?([a-z0-9]+\.)?[a-z0-9]+\.[a-z]{2,}(\/.*)?$/i;
	...
	return (
		<>
			<Appbar.Header>
				...
				/>
				<Button mode={'text'} buttonColor={theme.colors.secondary} onPress={() => handleSearch(inputText)}>
					Search
				</Button>
			</Appbar.Header>
			<WebView source={{ uri: url }} />
			{showMenu && (
				<Animated.View style={[styles.drawer, { transform: [{ translateY: bottomDrawerAnim.interpolate({
							inputRange: [0, 1],
							outputRange: [200, 0],
						}) }] }]}>
					<DrawerContentScrollView>
						<Drawer.Section>
							<DrawerItem label="Increase font size" onPress={() => console.log('Increasing font size')} />
							<DrawerItem label="Decrease font size" onPress={() => console.log('Decreasing font size')} />
						</Drawer.Section>
					</DrawerContentScrollView>
